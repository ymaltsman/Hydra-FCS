...
glsl:
`   
    _st = _st*2.0*amp - 1.0*amp;
    float x = _st.x;
    float y = _st.y;
    float u = pow(x*x + y*y - 2.0*a*x, 2.0) - 4.0*sin(time)*a*a*(x*x - y*y);
...
`   
    ...
    float x = _st.x;
    float y = _st.y;
    float u = y*y*(a*a - x*x) - pow(x*x + sin(time-y)*a*y - a, 3.0*a_exp);
    //u = 1.0-mod(u, wrap*10.0);
    return vec4(u, u, u, 1.0);`
})
//
setFunction({
...
`   
    ...
    float x = _st.x;
    float y = _st.y;
    float u = pow(pow(x, 2.0), .33) + pow(pow(y, 2.0), .33*sin(time-x))- pow(pow(amp*sin(time), 2.0), .33);
    u = mod(u, wrap*100.0);
    return vec4(u, u, u, 1.0);`
})
...
`   
    ...
    float x = _st.x;
    float y = _st.y;
    float u = pow(x*x + y*y, 2.0) - sin(time- mult*y*x)*2.0*a*a*(x*x - y*y) - pow(a, 4.0) + pow(c, 4.0);
...
`   
    ...
    float u = r*sin(1.1*time) - 4.0*a*pow(cos(theta/3.0), 3.0);
    u = fract(u+1.2*time);
    return vec4(u, u, u, 1.0);`
})
//
...
`   
    ...
    float r = length(_st);
    float theta = atan(y/x);
    float u = r - a*(sin(theta-time)/(theta));
    u = mod(u, 10.0*cos(time-y) + 10.0*sin(time-x));
    return vec4(u, u, u, 1.0);`
})
//
setFunction({
...
`   
    ...
    u = mod(u, wrap*sin(time-x)+wrap+1.0);
    return vec4(u, u, u, 1.0);`
})
//
setFunction({
...
`   
    ...
    float x = _st.x;
    float y = _st.y;
    float r = (c + a*cos(x-time))*cos(y);
    r = fract(r);
    float g = (c + a*cos(x+time))*sin(y);
    g = fract(g);
    float b = a*sin(x);
    b = fract(b-time);
    return vec4(r, g, b, 1.0);`
...
`   
    ...
    float r = length(_st);
    float theta = atan(y/x);
    float u = r - 4.0*a*cos(theta)*pow(sin(theta+time), 2.0);
    //u = 1.0-mod(u, 10.0);
    return vec4(u, u, u, 1.0);`
})
//
...
`   
    ...
    u = fract(u-time);
    //u = 1.0-mod(u, 10.0*sin(time)+10.0);
    return vec4(u, u, u, 1.0);`
})
//
setFunction({
...
`   
    ...
    float r = length(_st);
    float theta = atan(y/x);
    float u = r*r*cos(time-theta) - a*a*theta;
    return vec4(u, u, u, 1.0);`
})
//
setFunction({
...
setFunction({
    ...
    glsl: `
      ...
      float r = length(_st);
      float theta = atan(y / x);
      float u = r - 2.0 * a * (1.0 + sin(theta-2.0*time*y)); 
      return vec4(u, u, u, 1.0);
    `,
  })
  //
  ...
  setFunction({
    ...
    glsl: `
      ...
      float r = length(_st);
      float theta = atan(y/x);
      float u = r*r - cos(theta+c*time)*theta - 1.0;
      u = 1.0-mod(u, wrap);
      //u = fract(u-time);
      return vec4(u, u, u, 1.0);
    `,
